document.addEventListener('DOMContentLoaded', () => {
  const booksContainer = document.getElementById('borrowed-books-list');
  ...
  async function fetchBorrowedBooks() {
    try {
      const response = await fetch('/user/books/borrowed');
      ...
      booksContainer.innerHTML = '<p class="text-center text-red-500">Error loading borrowed books. Please try again.</p>';
    }
  }
  ...
  async function performSearch() {
    ...
    try {
      const response = await fetch(`/user/books/borrowed/search?query=${encodeURIComponent(query)}&filter=${currentFilter || ''}`);
      const data = await response.json();
